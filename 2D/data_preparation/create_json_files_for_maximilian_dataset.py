...
def main(argv):
    ...
    for row in rows:
        img_path = os.path.join(img_dir, row[0], row[1])
        label_path = os.path.join(label_dir, row[0], row[0]+'_Surfaces_Iowa.xml')
        bscan_num_for_png = int(row[2]) # iowa and tiff have different conventions for bscan numbers (n_png = n_tot - n_iowa)
